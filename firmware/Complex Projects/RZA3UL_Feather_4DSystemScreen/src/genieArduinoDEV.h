...
bool Genie::Begin_common() {
  ...
  if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Failed to detect display during setup\n");
  if ( UserHandler ) {
...
void Genie::AttachEventHandler(UserEventHandlerPtr userHandler) {
  ...
  if ( !displayDetected ) {
    if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Handler setup, display disconnected\n");
    uint8_t buffer[6] = { GENIE_DISCONNECTED, 0, 0, 0, 0 };
    _incomming_queue.push_back(buffer, 6);
  }
  else {
    if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Handler setup, display online\n");
...
int32_t Genie::ReadObject(uint8_t object, uint8_t index, bool now) {
  ...
  if ( !_outgoing_queue.replace(buffer,5,1,2,3) ) {
    if ( _outgoing_queue.size() == _outgoing_queue.capacity() ) if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Overflow writing frames to queue!\n");
  ...
  if ( millis() - autoPingTimer > ( ( displayDetected && !NAK_detected ) ? autoPing_swapSpeed = AUTO_PING_CYCLE : autoPing_swapSpeed = recover_pulse ) ) {
    ...
    if ( displayDetected && millis() - display_uptime > DISPLAY_TIMEOUT ) {
      display_uptime = millis();
       if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: disconnected by display timeout\n");
      uint8_t buffer[6] = { GENIE_DISCONNECTED, 0, 0, 0, 0 };
      _incomming_queue.push_back(buffer, 6);
      displayDetected = 0;
    }
    ...
    switch ( deviceSerial->peek() ) {
          ...
          if ( deviceSerial->available() >= 6 ) {
            ...
            if ( checksum == buffer[5] ) {
              if ( GENIE_OBJ_FORM == buffer[1] ) {
                ...
                if ( !displayDetected ) {
                  if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: online\n");
                  uint8_t buffer[6] = { GENIE_READY, 0, 0, 0, 0 };
                ...
                if ( NAK_detected ) {
                  if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Recovered from NAK(s)\n");
                ...
                if ( autoPingFlag ) {
                  autoPingFlag = 0;
                  if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: AutoPing success!\n");
                  display_uptime = millis();
                  return GENIE_REPORT_OBJ;
                }
                if ( pingRequest ) {
          ...
          if ( UserByteReader != nullptr ) {
            ...
            if ( magic_report_len > 0 ) {
              if ( debugSerial != false ) {
                  Serial.print((uint8_t*)"[Genie]: User forgot ");
                  Serial.print(magic_report_len);
                  Serial.print((uint8_t*)" magic byte(s). Flushing rest...\n");
              }
              uint32_t timeout = millis();
              ...
              if ( debugSerial != false ) {
                if ( !magic_overpull_count ) {
                    Serial.print((uint8_t*)"[Genie]: User captured all magic bytes!\n");
                } else {
                    Serial.print((uint8_t*)"[Genie]: User captured all magic bytes, but tried to pull more than provided! (");
                    Serial.print(magic_overpull_count);
                    Serial.print((uint8_t*)" byte(s))\n");
                }
              }
            }
            display_uptime = millis();
            ...
            if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: MMagic bytes callback not set!\n");
      ...
      case GENIEM_REPORT_DBYTES: {
          ...
          if ( UserDoubleByteReader != nullptr ) {
            ...
            if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Magic double bytes callback not set!\n");
      ...
      case GENIE_ACK: {
          deviceSerial->read();
          if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Received ACK!\n");
          pendingACK = 0;
          return GENIE_ACK;
        }
      case GENIE_NAK: {
          while ( deviceSerial->peek() == GENIE_NAK ) deviceSerial->read();
          if ( !genieStart && !NAK_detected && debugSerial != false ) Serial.print((uint8_t*)"[Genie]: Received NAK!\n");
          NAK_detected = 1;
      ...
      default: {
          if ( displayDetected && !NAK_detected && debugSerial != false ) {
              Serial.print((uint8_t*)"[Genie]: Bad Byte: ");
              Serial.println(deviceSerial->read());
          }
          break;
        }
    }
...
void Genie::dequeue_processing() {
  if ( pendingACK ) { /* check if ACK timeout, clear flag */
    if ( millis() - pendingACK_timeout >= 500 ) {
      if ( debugSerial != false ) Serial.print((uint8_t*)"[Genie]: ACK timeout!\n");
      pendingACK = 0;
    }
  }
