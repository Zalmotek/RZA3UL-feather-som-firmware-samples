...
T Genie_Buffer<T,_size,multi>::list() {
  if ( multi ) {
    if ( !size() ) {
        Serial.print((uint8_t*)"There are no queues available...\n"); return 0;
    }

    Serial.print((uint8_t*)"\nCircular Array Buffer Queue Size: "); Serial.print(size()); Serial.print((uint8_t*)" / "); Serial.println(_size);
    ...
    for ( uint16_t i = 2; i <= (((int)_cabuf[((head)&(_size-1))][0] << 8*sizeof(T)) | (int)_cabuf[((head)&(_size-1))][1])+1; i++ ) {
      ...
      Serial.print((uint8_t*)"    ");
    } Serial.print((uint8_t*)"("); Serial.print((((int)_cabuf[((head)&(_size-1))][0] << 8*sizeof(T)) | (int)_cabuf[((head)&(_size-1))][1])); Serial.print((uint8_t*)" entries.)\n");

    Serial.print((uint8_t*)"Last Entry:  ");
    for ( uint16_t i = 2; i <= (((int)_cabuf[((head+size()-1)&(_size-1))][0] << 8*sizeof(T)) | (int)_cabuf[((head+size()-1)&(_size-1))][1])+1; i++ ) {
      ...
      Serial.print((uint8_t*)"    ");
    } Serial.print((uint8_t*)"("); Serial.print((((int)_cabuf[((head+size()-1)&(_size-1))][0] << 8*sizeof(T)) | (int)_cabuf[((head+size()-1)&(_size-1))][1])); Serial.print((uint8_t*)" entries.)\n");
    ...
    for ( uint16_t i = 0; i < size(); i++ ) {
      ...
      for ( uint16_t j = 2; j <= (((int)_cabuf[((head+i)&(_size-1))][0] << 8*sizeof(T)) | (int)_cabuf[((head+i)&(_size-1))][1])+1; j++ ) {
        if ( (int)(_cabuf[((head+i)&(_size-1))][j]) != (T)(_cabuf[((head+i)&(_size-1))][j]) ) { // possible float?
            Serial.print(_cabuf[((head+i)&(_size-1))][j]); Serial.print((uint8_t*)"\t");
        }
        else {
            Serial.print(_cabuf[((head+i)&(_size-1))][j]); Serial.print((uint8_t*)"\t");
        }
      } Serial.print((uint8_t*)"("); Serial.print((((int)_cabuf[((head+i)&(_size-1))][0] << 8*sizeof(T)) | (int)_cabuf[((head+i)&(_size-1))][1])); Serial.print((uint8_t*)" entries.)\n");
    }
    return _available;
  }
  ...
  Serial.print((uint8_t*)"\nCircular Ring Buffer Queue Size: "); Serial.print(size()); Serial.print((uint8_t*)" / "); Serial.println(_size);

  Serial.print((uint8_t*)"\nIndice:  \t");

  for ( uint16_t i = 0; i < _available; i++ ) {
      Serial.print((uint8_t*)"[");
      Serial.print((head+i)&(_size-1)); Serial.print((uint8_t*)"]      \t");
  }
  ...
  for ( uint16_t i = 0; i < _available; i++ ) {
    if ( (int)_cbuf[((head+i)&(_size-1))] != (T)_cbuf[((head+i)&(_size-1))] ) { // possible float?
        Serial.print(_cbuf[((head+i)&(_size-1))]); Serial.print((uint8_t*)"\t");
    }
    else {
        Serial.print(_cbuf[((head+i)&(_size-1))]); Serial.print((uint8_t*)"\t\t");
    }
