...
static fsp_err_t wire_init_communication(uint8_t address) {
    ...
    err = I2CStart();
    if(err == FSP_SUCCESS) {
        err = R_RIIC_MASTER_SlaveAddressSet(&g_i2c_master0_ctrl, address, I2C_MASTER_ADDR_MODE_7BIT);
        if (FSP_SUCCESS != err) {
            //Serial.print((uint8_t*)"** I2C Slave Address Set Failed ** \r\n");
            return err;
        }
    }
    else {
...
static fsp_err_t wire_read(uint8_t* buff, uint32_t len, bool restart) {
    ...
    if (err == FSP_SUCCESS) {
        ...
        if (err != FSP_SUCCESS) {
            //Serial.print((uint8_t*)"** wire_read failed ** \r\n");
            return err;
        }
        rx_idx = len;
    }
...
static fsp_err_t requestFrom(uint8_t devAddr, uint32_t count) {
    ...
    if (err != FSP_SUCCESS) {
        //Serial.print((uint8_t*)"** requestFrom failed ** \r\n");
    }
    return err;
}
...
static fsp_err_t beginTransmission(uint8_t address) {
    ...
    if (err != FSP_SUCCESS) {
        //Serial.print((uint8_t*)"** beginTransmission failed ** \r\n");
        return err;
    }
    return err;
}
...
static fsp_err_t write(uint8_t data) {
    if (tx_idx < I2C_BUFF_TX_SIZE) {
        ...
        //Serial.print((uint8_t*)"** write failed ** \r\n");
