...
static fsp_err_t I2CStart() {
    //R_RIIC_MASTER_Close(&g_i2c_master0_ctrl);
    fsp_err_t err = FSP_SUCCESS;
    if (!i2c_started) {
        err = R_RIIC_MASTER_Open(&g_i2c_master0_ctrl, &g_i2c_master0_cfg);
        i2c_started = true;
    }
    if (FSP_SUCCESS != err) {
        //Serial.print((uint8_t*)"** R_RIIC_MASTER_Open API failed ** \r\n");
    }
    return err;
}

static fsp_err_t I2CRead(uint8_t *buff, uint32_t buff_len, uint8_t restart) {
    fsp_err_t err = R_RIIC_MASTER_Read(&g_i2c_master0_ctrl, buff, buff_len, restart);
    validate_i2c_event();
    if (FSP_SUCCESS != err) {
        //Serial.print((uint8_t*)"** I2C Read Failed ** \r\n");
    }
    return err;
}

static fsp_err_t I2CWrite(uint8_t *buff, uint32_t buff_len, uint8_t restart) {
    fsp_err_t err = R_RIIC_MASTER_Write(&g_i2c_master0_ctrl, buff, buff_len, restart);
    validate_i2c_event();
    if (err != FSP_SUCCESS) {
        //Serial.print((uint8_t*)"** I2C Write Failed ** \r\n");
    }
    return err;
}
